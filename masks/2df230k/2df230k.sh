...
echo "Hello $name, watch me make harmonics for the 2dF 230k survey."
...
echo "${mangledir}weight $quiet $mtol -z2dF230k jnb jnw"
${mangledir}weight $quiet $mtol -z2dF230k jnb jnw || exit
...
echo "Data suitable for plotting polygons of the 2dF 230k mask in matlab with the mapping toolbox are in $list."
...
if [ "$quiet" ]; then
    ...
    echo "please turn off the quiet button in 2df230k and try again."
fi
