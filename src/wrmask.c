...
int wr_poly(char *filename, format *fmt, int npolys, polygon *polys[/*npolys*/], int npolyw)
{
    ...
    FILE *file;
    char *poly_fmt;
    char *polygon_fmt = "polygon %d ( %d caps, %.15lg weight, %d pixel, %.15lf str):\n";
    char *spolygon_fmt = "%d %d %.15lg %d %.15lf\n";
...
/*------------------------------------------------------------------------------
  Write HEALPix weights.
   ...
   Return value: number of weights written,
		or -1 if error occurred.
*/
int wr_healpix_weight(char *filename, format *fmt, int numweight, double weights[/*numweight*/])
{
#undef	PRECISION
#define	PRECISION	6
	...
	if (weightmax < fabs(weights[iweight])) weightmax = fabs(weights[iweight]);
    }
    precision = (fmt->outprecision >= 0)? fmt->outprecision : PRECISION;
