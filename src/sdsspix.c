...
double pi, deg2Rad, rad2Deg, strad2Deg, etaOffSet;
...
{
  extern unsigned long nx0, ny0;
  extern double pi, deg2Rad, rad2Deg, strad2Deg; 
  long nx, ny, i, j;
...
void pix2ang_radec(int resolution, unsigned long pixnum, double *ra, double *dec) {

  void csurvey2eq(double lambda, double eta, double *ra, double *dec);
  double lam, eta, x, y, z, ra_tmp, dec_tmp;
...
void ang2pix_radec(int resolution, double ra, double dec, unsigned long *pixnum) {

  void eq2csurvey(double ra, double dec, double *lambda, double *eta);
  unsigned long tmp_pixnum;
  double x, y, z, lambda, eta;
		...
		unsigned long *y_min, unsigned long *y_max)
{
  ...
  unsigned long nx, ny, i, j, k, pixnum;
  double eta2;
