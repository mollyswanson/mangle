...
polygon *get_pixel(int pix, char scheme){
  ...
  double angle[4], lammin_c[4], lammax_c[4], etamin_c[4], etamax_c[4];
  azel v[4],v_r[4];
  polygon *pixel;
  ...
  if(scheme=='s'){
	...
	if(pix==9+18*i || pix==113){
      ...
      if(lammin < -90.) lammin += 180.;
      if(lammax > 90.) lammax -= 180.;
	...
	azel_to_rp(&(v_r[i]), pixel->rp[i]);
      ...
      pix_bound((int)pow(2,res-2), (unsigned long)pix - (unsigned long)pixel_start(res, scheme), &lammin, &lammax, &etamin, &etamax);
...
int get_child_pixels(int pix_p, int pix_c[], char scheme){
  ...
  if(scheme=='s'){
	...
	if(pix_p>=i && pix_p<=i+17) {
      ...
      subpix((int)pow(2,res-2), (unsigned long)(pix_p-pixel_start(res, scheme)), &pix_c0, &pix_c1, &pix_c2, &pix_c3);
...
int get_res(int pix, char scheme){
  ...
  /* else if(scheme=='h'){
    if(pix==0){
      ...
      for(res=2;pix>(int)(12*(pow(4,res)-4)/12);res++){
      }
    return((int)(res-1));  
    }
    } */
    ...
    for(res=2;pix>(int)pow(4,res-2)*468;res++){
      pix-=(int)pow(4,res-2)*468;
    }
    // sdss_res increases by factors of 2 instead of increments of 1
    // sdss_res = (int)pow(2,res-1);
    return(res);
  }
  else{
