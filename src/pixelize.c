...
/* getopt options */
//const char *optstr = "dqa:b:t:y:m:s:e:v:p:P:i:o:";
const char *optstr = "dqm:s:e:v:p:P:i:o:H";
...
{
  int ifile, nfiles, npoly, npolys, i, flag;
  clock_t start, stop;
  ...
  msg("pixelization took %g seconds.\n", (double)(stop-start)/CLOCKS_PER_SEC);
...
{
  printf("usage:\n");
  //  printf("pixelize [-d] [-q] [-a<a>[u]] [-b<a>[u]] [-t<a>[u]] [-y<r>] [-m<a>[u]] [-s<n>] [-e<n>] [-vo|-vn] [-p[+|-][<n>]] [-P[scheme][<r>][,<p>]] [-i<f>[<n>][u]] [-o<f>[u]] polygon_infile1 [polygon_infile2 ...] polygon_outfile\n");
 printf("pixelize [-d] [-q] [-m<a>[u]] [-s<n>] [-e<n>] [-vo|-vn] [-p[+|-][<n>]] [-P[scheme][<p>][,<r>]] [-i<f>[<n>][u]] [-o<f>[u]] polygon_infile1 [polygon_infile2 ...] polygon_outfile\n");
#include "usage.h"
}
...
#define WARNMAX			8
  ...
  for (i = 0; i < npoly; i++) {
    ...
    /* zero area polygon */
    if (iprune >= 2) {
      if (WARNMAX > 0 && inull == 0) msg("warning from pixelize: following polygons have zero area & are being discarded:\n");
      if (inull < WARNMAX) {
  ...
  n=pixel_loop(0,np,poly,npolys,polys);
  if(n==-1) return(-1);
