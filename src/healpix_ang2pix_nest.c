...
void healpix_ang2pix_nest( const int nside, double theta, double phi, int *ipix) {
  ...
  if( theta < 0. || theta > PI ) {
    fprintf(stderr, "healpix_ang2pix_nest: theta out of range: %f\n", theta);
    exit(0);
  }
  if( !setup_done ) {
  ...
  *ipix =(int)( ipf + face_num*pow(nside,2)); /* in {0, 12*nside**2 - 1} */
}
