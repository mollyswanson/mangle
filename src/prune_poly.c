...
int prune_poly(polygon *poly, double mtol)
{
    ...
    tol = mtol;
    if(tol > 0.01) printf("prune_poly: tol = %f\n", tol);
    verb = 1;
    ier = garea(poly, &tol, verb, &area_tot);
    if (ier) {printf("prune_poly: received error message from garea, tol = %f (1)\n", tol); return(-1); }
	...
	if (ier == -1) {printf("prune_poly: received error message from garea (2)\n"); return(-1); }
