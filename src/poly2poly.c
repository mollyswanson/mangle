...
/* getopt options */
const char *optstr = "dqm:j:k:ns:e:v:p:i:o:H";
...
{
    int ifile, ipoly, nfiles, npoly, npolys, i, flag;
...
{
    printf("usage:\n");
    printf("poly2poly [-d] [-q] [-m<a>[u]] [-j[<min>][,<max>]] [-k[min][,<max>]] [-n] [-s<n>] [-e<n>] [-vo|-vn] [-p[+|-][<n>]] [-i<f>[<n>][u]] [-o<f>[u]] [-H] polygon_infile1 [polygon_infile2 ...] polygon_outfile\n");
#include "usage.h"
}
