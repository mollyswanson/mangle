...
*/
    ...
    if (strchr(optstr, 'B')) printf("  -B\t\trasterize: polygon_infile2, polygon_infile3,... already balkanized\n");
	...
	printf("             \ts spolygon, v vertices\n");
       	printf("             \tinput only for healpix2poly: h healpix_weight\n");
    }
    if (strchr(optstr, 'o')) {
	printf("             \toutput only: a area, g<i> graphics, i id, m midpoint, w weight\n");
    }

    if (strchr(optstr, 'H')) printf("  -H\t\twrite polygon_outfile in correct format for later use in rasterize\n");
