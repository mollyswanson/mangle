...
void parse_args(int argc, char *argv[])
{
	...
	case 'd':		/* advise defaults */
		...
		if (strchr(optstr, 'o')) {
  	    ...
  	    if (strcmp(argv[0], "healpix2poly") == 0) {
	       printf("\nusing -ih is recommended (healpix_weight is only allowed input file format)\n");
	       printf("otherwise make sure your input file is formatted properly\n");
	    }
	    printf("\n");
	    exit(0);
	case 'q':		/* be quiet */
	    verbose = 0;
	    break;
	case 'B':               /* polygon_infile2, polygon_infile3, etc. already balkanized prior to rasterize */
	    pre_balk = 1;
	    break;
	...
	case 'o':		/* format of output file */
		...
		break;
	    case 'h':   fmt.out = keywords[HEALPIX_WEIGHT];   break;
	    case 'l':
