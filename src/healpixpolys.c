...
polygon *get_healpix_poly(int nside, int hpix)
{
  ...
  azel verts1, verts2, verts3, verts4, v[4];
  vertices *vert;
  polygon *pixel, *pixelbetter;
  double verts_vec[12], verts_vec_n[12], verts_vec_e[12], verts_vec_s[12], verts_vec_w[12], dist_n, dist_w, dist_s, dist_e;
...
/*------------------------------------------------------------
  ...
  Input: nweights = number of weights in HEALPix_weights input file
...
void healpix_verts(int nside, int pix, vec center, double verts[12])
{
...
/*-------------------------------------------------------------
  cmrpirpj: C version of Fortran subroutine that calculates the value
         of (1-cos(th(ij))), where th(ij) is the angle between the
	 unit vectors rpi and rpj

  Input: rpi, rpj = unit vectors
  Return value: 1- cos(th(ij))
*/

double cmrpirpj(vec rpi, vec rpj)
{
  double cmij;
  
  cmij = (pow((rpi[0]-rpj[0]),2)+pow((rpi[1]-rpj[1]),2)+pow((rpi[2]-rpj[2]),2))/2.;
  return(cmij);
}
