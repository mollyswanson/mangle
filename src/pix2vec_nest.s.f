!     Standalone HEALPix subroutine converted to f77 by James C Hill 07/13/06
...
!     a f 'f88 pix2vecnest.f'
     ...
     & vertex_w_x, vertex_w_y, vertex_w_z)
      ...
      IMPLICIT none
      INTEGER ipix, nside
      REAL*8 vector_x, vector_y, vector_z, vertex_n_x, 
     ...
     & ip_hi, jrt, jr, nr, jpt, jp, kshift, nl4, ns_max
      ...
      REAL*8 z, fn, fact1, fact2, sth, phi, PI
      ...
      REAL*8 phi_nv, phi_wv, phi_sv, phi_ev, phi_up, phi_dn, 
...
!               if (id_mk.ge.2 .or. id_mk.lt.0) goto 20  ! bit value (in kpix), goes in iy_mk
      ...
      phi = (jp - (kshift+1)*0.5d0) * (PI/(2.0d0*nr))
      ...
      if (nr.gt.1) phi_up=(PI/2.0d0)*(iphi_rat+iphi_mod/(DBLE(nr-1)))
      phi_dn         =(PI/2.0d0)*(iphi_rat+(iphi_mod+1)/(DBLE(nr+1)))
