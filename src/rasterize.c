...
int main(int argc, char *argv[])
{
  ...
  int ifile, ipoly, nfiles, npoly, npolys, nhealpix_poly, nhealpix_polys, nrast_poly, nrast_polys, i, j, verb, ier, imid, ivm, nev, nev0, nv, nvm, ier_r, ier_h, ipnest, nside, nadj, numweight, nweight;
  int *ipv, *gp, *ev;
  long double *weights, *angle, area_r, area_h, tol;
  vec *ve, *vm;
  azel v;
  char key;
  ...
  nrast_poly = nhealpix_poly + npoly;
  ...
  /* snap polygons before balkanizing */
  tol = mtol;
  nadj = snap(nrast_poly, rast_polys, tol, &fmt, axtol, btol, thtol, ytol, 0);
  if(nadj==-1) exit(1);
  ...
  weights = (long double *) malloc(sizeof(long double) * (fmt.nweights));
  if (!weights) {
  ...
  //area_h = (4*PI)/(long double)(fmt.nweights);
  ...
  /* write weights */
  ifile = argc - 1;
  numweight = fmt.nweights;
  nweight = wr_healpix_weight(argv[ifile], &fmt, numweight, weights);
  if (nweight == -1) exit(1);
