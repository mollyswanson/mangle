...
int pixelmap(int *npoly, polygon *poly[/**npoly*/])
{
  ...
  if(res1<res_max){
    fprintf(stderr,"pixelmap: there are pixels in the mask with a lower resolution than the desired pixelmap resolution %d.  The desired pixelmap resolution can be set wit the -P option.\n",res_max);
  ...
  av_weight0= (double *) malloc(sizeof(double) * (kend-kstart+1) );
  if (!av_weight0) {
    fprintf(stderr, "pixelmap: failed to allocate memory for %d integers\n", kend-kstart+1 );
    return(-1);
  }
  ...
  for(p=min_pixel;p<max_pixel;p++){
    ...
    //set k to the pixel at the desired output resolution
    k=parent_pixels[res_max];
